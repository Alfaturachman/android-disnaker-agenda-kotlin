...
class TambahAgendaActivity : AppCompatActivity() {
    ...
    private fun simpanMediasi() {
        ...
        RetrofitClient.instance.tambahPelapor(request).enqueue(object :
            Callback<ApiResponse<TambahPelapor>> {
            override fun onResponse(call: Call<ApiResponse<TambahPelapor>>, response: Response<ApiResponse<TambahPelapor>>) {
                if (response.isSuccessful) {
                    ...
                    if (result != null && result.status) {
                        ...
                        Toast.makeText(this@TambahAgendaActivity, "Mediasi berhasil disimpan!", Toast.LENGTH_SHORT).show()

                        val intent = Intent(this@TambahAgendaActivity, RiwayatPelaporActivity::class.java)
                        startActivity(intent)
                        finish()
                    } else {
                        val errorBody = response.errorBody()?.string()
                        Log.e("TambahPelaporActivity", "Gagal menyimpan mediasi. Error: $errorBody")
                        Toast.makeText(this@TambahAgendaActivity, "Gagal menyimpan mediasi!", Toast.LENGTH_SHORT).show()
                    }
                } else {
                    Log.e("TambahPelaporActivity", "Response error: ${response.code()} - ${response.message()}")
                    Toast.makeText(this@TambahAgendaActivity, "Error: ${response.message()}", Toast.LENGTH_SHORT).show()
                }
            }
            ...
            override fun onFailure(call: Call<ApiResponse<TambahPelapor>>, t: Throwable) {
                ...
                Toast.makeText(this@TambahAgendaActivity, "Gagal menghubungi server!", Toast.LENGTH_SHORT).show()
            }
        })
    }
